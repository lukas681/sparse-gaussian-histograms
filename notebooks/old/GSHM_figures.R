...
#Root finder for mu:
# based on newtons method to find the root.
findrootmu = function(candidate_mu, epsilon, delta){
    ...
    for(i in 1:length(m)){
        ...
        fp = dnorm(mu/2 - epsilon/mu)*(1/2 + epsilon/mu^2) - exp(epsilon)*dnorm(-mu/2 - epsilon/mu)*(-1/2 + epsilon/mu^2)
        mu = mu - f/fp
        m[i] = mu
    }
    return(mu)
...
gshm_delta <- function(c_u, sigma, tau_diff, epsilon){
    ...
    print(paste(tau_diff, " ",  c_u, " ", epsilon, " ", sigma))
    print(1 - pnorm(tau_diff/sigma)^c_u)
    final_delta = max(c(
...
## Set parameters
c_u = 51914
eps_views = 0.347 # Epsilon for the thresholding column
delta = 10^(-5)
k = sqrt(c_u)
...
mu = findrootmu(sqrt(eps_views), eps_views, delta) # I guess this checks the last equation.
print("here")
print(mu)
min_sigma = k/mu
...
print(sum(tau_diff_gshm_check)/length(tau_diff_gshm_check))
...
lines(sigma[seq(1, length(sigma), 10)], tau_diff_gshm[seq(1, length(sigma), 10)], col="blue", lwd=2, lty=3) # The plotted lines.

# sigma: the x axis.
# tau_diff might be add-the-delta tau.
# tau_diff_gsh: Computed tau for the exact analysis.
